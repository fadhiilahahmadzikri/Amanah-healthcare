...
export function CustomerEditPage({ customerId }: CustomerEditPageProps) {
  ...
  const [form, setForm] = useState<CustomerFormState>(() => getInitialFormState(customer));
  ...
  const handleSave = () => {
    ...
    updateCustomer(customer.id, buildCustomerPayload(form));
  ...
  return (
    <div className='flex max-w-3xl flex-col gap-4'>
      <div>
        <h2 className='text-3xl font-bold tracking-tight'>Edit: {customer.name}</h2>
...
function getInitialFormState(customer?: Customer): CustomerFormState {
  if (!customer) {
    return {
      name: '',
      email: '',
...
function buildCustomerPayload(form: CustomerFormState): CustomerUpdateValues {
  return {
    name: form.name.trim(),
